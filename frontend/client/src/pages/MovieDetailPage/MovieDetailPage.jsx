...
import { ArrowLeft, Calendar, Clock, Star, Play, X, Users, User } from "lucide-react";
import movies from "../../components/MoviesPage/dummydata";
...
const formatTimeInTZ = (dateLike, timeZone = "Asia/Kolkata") => {
  ...
  return `${hour}:${p.minute} ${String(p.dayPeriod ?? p.ampm ?? "").toUpperCase()}`;
};
...
export default function MovieDetailPage() {
  ...
  const showtimeDays = useMemo(() => {
    ...
    (movie.slots || []).forEach((slot) => {
      try {
        ...
        if (typeof slot === "string") {
          ...
          audi = slot.audi || slot.audiName || slot.auditorium || slot.auditoriumName || null;
        }
    ...
    const days = dateKeys.map((key) => {
      ...
      const dayName = new Intl.DateTimeFormat("en-US", { weekday: "long", timeZone: TZ }).format(asDate);
      ...
      const showtimes = rawSlots
        .map(({ iso, audi }) => {
          ...
          return { time: timeLabel, datetime: iso, timestamp: d.getTime(), audi: audi ?? null };
        })
        .filter(Boolean)
    ...
    <div className={movieDetailStyles.container}>
      ...
      <div className={movieDetailStyles.wrapper}>
        ...
        <div className={movieDetailStyles.header}>
          <Link
            to="/movies"
            className={movieDetailStyles.backButton}
          >
            <ArrowLeft size={18} /> <span className={movieDetailStyles.backText}>Back</span>
          </Link>
        </div>
        ...
        <div className={movieDetailStyles.mainLayout}>
          ...
          {/* Middle + Right: Showtimes & Cast (span 2 columns on lg) */}
          <div className={movieDetailStyles.rightColumns}>
            ...
            <div className={movieDetailStyles.showtimesCard}>
              ...
              {/* Day Selection - horizontal scroll on small screens */}
            ...
            <div className={movieDetailStyles.castCard}>
              <h3 className={movieDetailStyles.castTitle} style={{ fontFamily: "'Cinzel', serif" }}>
                <Users className={movieDetailStyles.castIcon} />
                <span>Cast</span>
              </h3>
        ...
        <div className={movieDetailStyles.crewGrid}>
          ...
          <div className={movieDetailStyles.crewCard}>
            ...
            <div className={movieDetailStyles.crewContent}>
              ...
              <div className={movieDetailStyles.crewName}>{movie.producer?.name ?? "N/A"}</div>
            </div>
          </div>
        </div>
